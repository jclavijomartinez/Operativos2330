...
struct DatosSolicitud {
    ...
    int hora;
    int numPersonas;
    int respuesta; // Nueva variable para la respuesta al agente (0: denegada, 1: positiva, 2: reprogramación)
};
...
void *recibirSolicitud(void *datosPipe) {
    ...
    while (horaActual < horaFinal || read(pipe_fd, &datosSolicitud, sizeof(struct DatosSolicitud)) > 0) {
        ...
        if (datosSolicitud.hora < horaActual) {
            ...
            printf("Solicitud denegada: %s, Hora: %d, Personas: %d (Hora inválida)\n", datosSolicitud.nombre, datosSolicitud.hora, datosSolicitud.numPersonas);
            solicitudesNegadas++;
        } else {
            // Verificar la capacidad antes de autorizar la solicitud
            if (numVisitantesPorHora[datosSolicitud.hora % 24] + datosSolicitud.numPersonas <= totalPersonas) {
                numVisitantesPorHora[datosSolicitud.hora % 24] += datosSolicitud.numPersonas;
                ...
                printf("Solicitud reprogramada: %s, Hora: %d, Personas: %d (Capacidad excedida)\n", datosSolicitud.nombre, datosSolicitud.hora, datosSolicitud.numPersonas);
            }
        }
    ...
    if (horaActual >= horaFinal) {
        ...
        printf("El hilo de solicitudes se ha cerrado debido a un error o finalización prematura.\n");
    }
...
int main(int argc, char *argv[]) {
    ...
    if (horaI == 0 || horaF == 0 || segHora == 0 || totalPersonas == 0 || nomPipe == 0) {
        printf("Algunos de los argumentos no son válidos.\n");
        exit(1);
    }

    if (horaI == -1 || horaF == -1 || segHora == -1 || totalPersonas == -1 || nomPipe == -1) {
    ...
    if (pthread_create(&hilo_tiempo, NULL, esperaHora, (void *)&datosEspera) != 0) {
    ...
    if (pthread_create(&hilo_solicitudes, NULL, recibirSolicitud, (void *)&datosPipe) != 0) {
